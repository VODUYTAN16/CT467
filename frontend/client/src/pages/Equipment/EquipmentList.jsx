...
const EquimentList = () => {
  ...
  const fetch = async () => {
    ...
    try {
      ...
      setSnack({
        ...
        severity: 'error',
        message:
          t('dashboard.error_loading_equipment') || 'Error loading equipment',
      });
    } finally {
      setLoading(false);
    }
  ...
  const handleDelete = async (id) => {
    ...
    try {
      ...
      setSnack({
        open: true,
        severity: 'success',
        message: t('message.deleted') || 'Deleted',
      });
      fetch();
    } catch (err) {
      console.error(err);
  ...
  const columns = [
    ...
    {
      field: 'status',
      headerName: t('equipment.status') || 'Status',
      width: 140,
    },
    {
      field: 'actions',
      ...
      renderCell: (params) => (
        <Box sx={{ display: 'flex', gap: 1 }}>
          <Tooltip title={t('button.edit') || 'Edit'}>
            <IconButton
              size="small"
              onClick={() => navigate(`/equipment/edit/${params.row.id}`)}
    ...
    <Container maxWidth="lg" sx={{ py: 3 }}>
      ...
      >
        <Typography variant="h5">
          {t('equipment.title') || t('sidebar.equipment')}
        </Typography>
        <Box sx={{ display: 'flex', gap: 2 }}>
          ...
          >
            {t('equipment.add_equipment') || t('form.add_member') || 'Add'}
          </Button>
        </Box>
      </Box>
