  ...
  const fetchMembersAndEquipment = async () => {
    try {
      ...
      setSnack({
        ...
        severity: 'error',
        message:
          t('usage.error_loading_dependencies') ||
          'Error loading members or equipment',
      });
    }
  };
    ...
    try {
      const payload = {
        ...
        use_date: formData.use_date
          ? new Date(formData.use_date).toISOString().slice(0, 10)
          : null,
        start_time: formData.start_time
          ? new Date(formData.start_time).toLocaleTimeString('en-GB', {
              hour: '2-digit',
              minute: '2-digit',
              second: '2-digit',
            })
        ...
        end_time: formData.end_time
          ? new Date(formData.end_time).toLocaleTimeString('en-GB', {
              hour: '2-digit',
              minute: '2-digit',
              second: '2-digit',
            })
      ...
      setSnack({
        open: true,
        severity: 'success',
        message: t('message.saved') || 'Usage record created successfully',
      });
