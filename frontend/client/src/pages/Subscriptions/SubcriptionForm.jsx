...
import { TextField, Button, Paper, Box, Container, Grid, MenuItem, Snackbar, Alert, CircularProgress, Typography, List, ListItem, ListItemIcon, ListItemText } from "@mui/material";
...
const SubscriptionForm = () => {
      ...
      try {
        ...
        if (isEditMode) {
          ...
          setFormData({
            ...
            start_date: subRes.data.start_date ? new Date(subRes.data.start_date) : null,
          });
          if (packageIdFromSub) {
            const packageDetailsRes = await api.get(`/packages/${packageIdFromSub}`);
          ...
          if (subRes.data.member_id) {
            const memberRes = await api.get(`/members/${subRes.data.member_id}`);
  ...
  useEffect(() => {
    const fetchMember = async () => {
      ...
      try {
        ...
        setSelectedMember(memberRes.data);
        setFormData((prev) => ({ ...prev, member_id: memberRes.data.member_id }));
      } catch (error) {
  ...
  const handleChange = (e) => {
    ...
    if (name === "memberPhone") {
      ...
      setFormData((prev) => ({ ...prev, [name]: name === "package_id" ? Number(value) : value }));
    }
  };
  ...
  const handleSubmit = async (e) => {
    ...
    try {
      const payload = {
        ...formData,
        start_date: formData.start_date ? new Date(formData.start_date).toISOString().slice(0, 10) : null,
      };
  ...
  return (
    <Container maxWidth="lg">
      <PageHeader title={isEditMode ? t("subscriptions.edit_title") : t("subscriptions.add_title")} />
          ...
          <Grid container spacing={4}>
            <Grid xs={12} md={6}>
              <Grid container spacing={3}>
                ...
                <Grid xs={12}>
                  <TextField fullWidth label={t("subscriptions.package")} name="package_id" select value={formData.package_id} onChange={handleChange} required disabled={!selectedMember}>
                    {packages.map((pkg) => (
                ...
                <Grid xs={12}>
                  <LocalizationProvider dateAdapter={AdapterDateFns}>
                    <DatePicker label={t("subscriptions.start_date")} value={formData.start_date} onChange={handleDateChange} slotProps={{ textField: { fullWidth: true, required: true } }} />
                ...
                <Box sx={{ bgcolor: "background.paper", borderRadius: 2, p: 3, height: "100%" }}>
                  ...
                  <List>
                    <ListItem>
                      ...
                      <ListItemText primary={t("packages.price")} secondary={formatCurrencyVND(selectedPackageDetails.price)} primaryTypographyProps={{ fontWeight: "medium" }} secondaryTypographyProps={{ fontSize: "1.1rem", color: "text.primary" }} />
                    </ListItem>
                    <ListItem>
                      <ListItemIcon>
                        <CalendarToday />
                      ...
                      <ListItemText
                        ...
                        secondary={selectedPackageDetails.duration_months !== null && selectedPackageDetails.duration_months !== undefined ? `${selectedPackageDetails.duration_months} ${t("packages.months")}` : t("common.no_data")}
                        primaryTypographyProps={{ fontWeight: "medium" }}
                        secondaryTypographyProps={{ fontSize: "1.1rem", color: "text.primary" }}
                      />
                    </ListItem>
                    <ListItem>
                      <ListItemIcon>
                      ...
                      <ListItemText
                        ...
                        secondary={selectedPackageDetails.sessions_per_week !== null && selectedPackageDetails.sessions_per_week !== undefined ? String(selectedPackageDetails.sessions_per_week) : t("common.no_data")}
                        primaryTypographyProps={{ fontWeight: "medium" }}
                        secondaryTypographyProps={{ fontSize: "1.1rem", color: "text.primary" }}
                      />
                    </ListItem>
