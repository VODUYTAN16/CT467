...
const SubscriptionList = () => {
  ...
  const handleDelete = async (id) => {
    if (
      window.confirm(
        t('message.confirm_delete_subscription') ||
      ...
      try {
        ...
        setSnack({
          open: true,
          severity: 'success',
          message: t('message.deleted') || 'Deleted successfully',
        });
        fetchSubscriptions();
      } catch (error) {
  ...
  const filteredSubscriptions = subscriptions.filter((sub) => {
    ...
    const endDate =
      getEndDate(
        sub.start_date,
        sub.package?.duration || 0
      )?.toLocaleDateString() || '';

    return [memberName, packageName, startDate, endDate]
      .join(' ')
      .toLowerCase()
  ...
  const filteredExpiringMembers = expiringMembers.filter((member) => {
    ...
    const memberInfo = `${member.full_name} ${member.phone} ${
      member.package_name
      ...
      renderCell: (params) => (
        <Box sx={{ display: 'flex', gap: 1 }}>
          <Tooltip title={t('button.edit') || 'Edit'}>
            <IconButton
              onClick={() => navigate(`/subscriptions/edit/${params.row.id}`)}
              size="small"
    ...
    <Container maxWidth="lg" sx={{ py: 3 }}>
      ...
      {expiringMembers.length > 0 && (
        ...
        >
          ...
          />
          <DataGrid
            rows={filteredExpiringMembers.map((member) => ({
              id: member.member_id,
              ...member,
            }))}
            columns={[
              ...
              {
                field: 'full_name',
                headerName: t('members.full_name'),
                flex: 1.5,
                minWidth: 150,
              },
              {
                field: 'phone',
                headerName: t('members.phone'),
                flex: 1,
                minWidth: 100,
              },
              {
