...
export async function list(req, res, next) {
  try {
    ...
    let sql = `SELECT pay.*, s.member_id, s.package_id, m.full_name as member_name, m.phone as member_phone, p.name as package_name
               FROM payments pay
...
export async function getOne(req, res, next) {
  try {
    ...
    const [[row]] = await pool.query(
      `SELECT pay.*, s.member_id, s.package_id, m.full_name as member_name, m.phone as member_phone, p.name as package_name
       FROM payments pay
       JOIN subscriptions s ON s.subscription_id = pay.subscription_id
       JOIN members m ON m.member_id = s.member_id
       JOIN packages p ON p.package_id = s.package_id
